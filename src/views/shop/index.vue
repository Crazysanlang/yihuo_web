    ...
    <div class="list">
      ...
      <div class="tableCont">
        <el-table v-loading="loading" style="width: 100%" :data="list" element-loading-text="Loading">
          ...
          <el-table-column label="库存" align="left">
            <template slot-scope="scope">
    ...
    >
      <el-form ref="form" size="small" :model="dialogForm" :rules="rules" label-width="80px">
        ...
        <el-form-item label="库存" prop="price">
          <el-input v-model="dialogForm.last" />
        </el-form-item>
...
<script>
import { getShopList, addShopList, deleteShopList, updateShopList, listingNft } from '@/api/table'
        ...
        if (valid) {
          this.$confirm(`确定${this.isAdd ? '新增' : '修改'}, 是否继续?`, '提示', {
            ...
            const params = {
              ...
              last: Number(this.dialogForm.last),
              count: Number(this.dialogForm.last)
            }
            if (this.isAdd) {
              addShopList(params).then(() => {
                this.$message.success('操作成功！')
                this.dialogForm = {
                  name: '',
                  desc: '',
